...
public class MarathonController {
	...
	private int PERIOD_INTERVAL = 1000;
	...
	private void startclicked(ActionEvent ev) {
        ...
        newPiece();
		start();
		Marathon.scene.setOnKeyPressed(new EventHandler<KeyEvent>() {			
	...
	private void newPiece() {
		...
		if (!tryMove(curPiece, curX, curY, true)) {
            ...
            gamecycle.cancel();
			Platform.runLater(() -> top.setText("Your score: " + totalscore));
        }
	}
	...
	private void removeFullLines() {
        ...
        if (numFullLines > 0) {
			...
			totalscore += bonus * numFullLines;
            Platform.runLater(() -> score.setText(String.valueOf(totalscore)));
            isFallingFinished = true;
            curPiece.setShape(Tetrominoe.NoShape);
			if (PERIOD_INTERVAL > 30) {
				level++;
				Platform.runLater(() -> top.setText("Level " + String.valueOf(level)));
				bonus += 10;
				PERIOD_INTERVAL /= 2;
				gamecycle.cancel();
				start();
	...
	private void pause() {
        ...
        if (isPaused) {
			...
			top.setText("Level " + String.valueOf(level));
		}
        drawing();
    }
